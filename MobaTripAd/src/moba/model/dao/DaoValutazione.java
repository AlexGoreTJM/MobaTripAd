...
//Classe java contenente i dao della tabella Valutazione.
...
public class DaoValutazione extends DAO {
	...
	public <T> int insert(T entity) throws DAOException {
		...
		String sql = "INSERT INTO valutazione " + "(idutente,idgioco,voto) " + "VALUES (?,?,?)";
	...
	public void delete(int idUtenteCancellazione, int idGiocoCancellazione) throws DAOException {
		...
		try (PreparedStatement pst = con.prepareStatement(sql)) {
			...
			throw new DAOException("ERRORE DELETE VALUTAZIONE x idUtente: " + idUtenteCancellazione + " e X idGioco "
					+ idGiocoCancellazione + ". Causa: " + e.getMessage() + " Errorcode: " + e.getErrorCode());
		}
	}
	...
	public Valutazione select(int idUtente, int idGioco) throws DAOException {
		String sql = "SELECT idutente, idgioco, voto " + "FROM RECENSIONE " + "WHERE idutente = ? and idgioco = ?";
		...
		try (PreparedStatement pst = con.prepareStatement(sql)) {
			...
			throw new DAOException("ERRORE SELECT GRADO x nome: " + idUtente + ". Causa: " + e.getMessage()
	...
	private Valutazione componiEntity() throws SQLException, DAOException {
		...
		return new Valutazione(new DaoUtente().select(res.getInt("idutente")),
				new DaoGioco().select(res.getInt("idgioco")), res.getInt("voto"));
	}
	...
	public double getAvgValutazioneByIdGioco(int idGioco) throws DAOException {
		...
		String sql = "SELECT avg(voto) as votoMedio " + "FROM valutazione " + "WHERE idgioco = ?";
		...
		try (PreparedStatement pst = con.prepareStatement(sql)) {
			...
			if (res.next())
				...
				throw new DAONonTrovatoException("WARNING: dati non trovati in VALUTAZIONE x idGioco " + idGioco);

		} catch (SQLException e) {
			throw new DAOException("ERRORE SELECT GRADO x nome: " + idGioco + ". Causa: " + e.getMessage()
					+ " Errorcode: " + e.getErrorCode());
		}
